...
export default function InformationContent({ teams }: Props) {
  const [openSection, setOpenSection] = useState<string | null>(null);
  const [openPot, setOpenPot] = useState<number | null>(null);

  const toggle = (section: string) => {
    setOpenSection(openSection === section ? null : section);
  };

  const pot1 = teams.filter((team) => team.pot === 1);
  const pot2 = teams.filter((team) => team.pot === 2);
  const pot3 = teams.filter((team) => team.pot === 3);
  const pot4 = teams.filter((team) => team.pot === 4);
          ...
          {openSection === "scoring" && (
            <div className="rounded-xl bg-zinc-950 px-8 py-6 text-lg leading-8 text-zinc-300">
              <ul className="space-y-3">
                ...
                <li>
                  Only Premier League and Scottish Premiership league goals count.
                </li>

                <li>
                  Cup, European, friendly and play-off goals do not count.
                </li>
              </ul>
            </div>
          )}
                ...
                {[1, 2, 3, 4].map((pot) => {
                  ...
                  return (
                    ...
                    >
                      <button
                        onClick={() =>
                          setOpenPot(openPot === pot ? null : pot)
                        }
                        className="flex w-full items-center justify-between px-5 py-4 text-left transition hover:bg-zinc-900"
                      >
                        ...
                        <div className="border-t border-zinc-800 px-5 py-4">
                          <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            {potTeams.map((team) => (
                              <li
                                key={team.team}
                                className="rounded-lg bg-zinc-900 px-4 py-3 text-zinc-200"
                              >
                                {team.team}
                              </li>
                            ))}
